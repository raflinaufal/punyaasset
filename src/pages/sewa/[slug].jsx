...
const ProductSewa = ({ initialProducts }) => {
  ...
  const filteredProducts = initialProducts
    .filter((product) => {
      ...
      const searchLower = filteredSearch ? filteredSearch.toLowerCase() : "";
      // Include only products that match the specified market
      const isMarketMatch = product.market.includes("Di Sewa");
      ...
      return (
        ...
        (!selectedCity || product.kota.some((k) => k.name === selectedCity)) &&
        isTypePropertyMatch &&
        // Check if the product's "market" includes "Di Sewa"

        JSON.parse(product.market).includes("Di Sewa")
      );
    })
    .filter((product) => product.user_id === "1"); /// Add this condition to filter by user_id
                      ...
                      <div className="mt-4">
                        <div className="flex mb-4 text-sm">
                          <div className="px-2 text-white rounded bg-bg">
                            {/* <div key={product.type.id}>{product.type.name}</div> */}
                          </div>
                        </div>
...
export async function getServerSideProps(context) {
  ...
  // Panggil API untuk mendapatkan data produk berdasarkan slug
  const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/type/${slug}`;
  // console.log(apiUrl);

  try {
    const response = await fetch(apiUrl);
    const typeData = await response.json();
    // Ambil data produk dari jenis tersebut (Anda dapat menyesuaikan ini sesuai kebutuhan)
    const initialProducts = typeData.type.products;
